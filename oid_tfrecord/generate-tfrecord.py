...
tf.compat.v1.flags.DEFINE_string('class_descriptions_file', None, 'Path to the CSV file with class id mapping to human readable label name')
tf.compat.v1.flags.DEFINE_string('annotations_file', None, 'Path to the CSV file with bbox annotations')
tf.compat.v1.flags.DEFINE_string('images_dir', None, 'Path to the directory with downloaded images')
tf.compat.v1.flags.DEFINE_string('output_file', None, 'Path to the resulting TFRecord file')
...
def main(_):
    ...
    writer = tf.compat.v1.python_io.TFRecordWriter(FLAGS.output_file)
